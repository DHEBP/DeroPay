 ...
 * Encodes text as a QR code and renders to a canvas element.
 * Based on the QR code algorithm with byte-mode encoding.
 */

const EC_L = 1;

export function renderQR(container: HTMLElement, text: string, size = 200): void {
...
function addFinderPatterns(m: boolean[][], r: boolean[][], s: number): void {
  const positions = [[0, 0], [0, s - 7], [s - 7, 0]];
...
function addAlignmentPatterns(m: boolean[][], r: boolean[][], v: number, s: number): void {
  if (v < 2) return;
  const positions = getAlignmentPositions(v);
...
function getAlignmentPositions(v: number): number[] {
  if (v === 1) return [];
  const last = v * 4 + 10;
  ...
  const result = [6];
  for (let i = last; result.length < count; i -= step) result.splice(1, 0, i);
  return result;
}
...
function encodeData(data: Uint8Array, version: number): number[] {
  ...
  push(bits, 0b0100, 4); // byte mode
  push(bits, data.length, version <= 9 ? 8 : 16);
  for (const b of data) push(bits, b, 8);
  push(bits, 0, Math.min(4, totalBits - bits.length));
  while (bits.length % 8 !== 0) bits.push(0);
  const padBytes = [0b11101100, 0b00010001];
  let pi = 0;
  while (bits.length < totalBits) {
    push(bits, padBytes[pi % 2], 8);
    pi++;
  }
  return bits;
}

function getDataCapacityBits(v: number): number {
  const caps = [0, 152, 272, 440, 640, 864, 1088, 1248, 1552, 1856, 2192, 2592, 2960, 3424, 3688, 4184, 4712, 5176, 5768, 6360, 6888];
...
function addFormatInfo(m: boolean[][], s: number): void {
  const formatBits = 0b101010000010010; // L, mask 0
  for (let i = 0; i < 15; i++) {
